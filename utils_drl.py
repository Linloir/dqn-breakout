...
class Agent(object):
    ...
    def learn(self, memory: ReplayMemory, batch_size: int, ps: float = 0.7) -> float:
        """learn trains the value network via TD-learning."""

        # Sample a batch of transitions from the replay memory
        state_batch, action_batch, reward_batch, next_batch, done_batch, indices_batch, importance_batch = \
            memory.sample(batch_size, ps)

        # Calculate the Q values of the next states
        values = self.__policy(state_batch.float()).gather(1, action_batch)     # Q(s, a) generated by the policy network
        values_next = self.__target(next_batch.float()).max(1).values.detach()  # max_a Q(s', a) generated by the target network
        expected_values = (self.__gamma * values_next.unsqueeze(1)) * (1. - done_batch) + reward_batch # Expected value of Q(s, a) which should be equal to gamma * max_a Q(s', a) + r
        importance = importance_batch ** (1 - self.__eps)                        # Importance-sampling weights
        # Calculate the loss
        errors = values - expected_values
        loss = (importance * errors ** 2).mean()

        # Update the priority of the sampled transitions
        memory.update_priorities(indices_batch, errors.detach().abs().squeeze(1))
